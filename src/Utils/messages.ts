...
export const prepareWAMessageMedia = async (
	...
	// ?? DEBUG: Log das condi��es para convers�o de �udio
	if (mediaType === 'audio') {
		logger?.debug({
			mediaType,
			transformAudio: options.transformAudio,
			ptt: uploadData.ptt,
			requiresAudioTransformation,
			originalMimetype: uploadData.mimetype
		}, '?? AUDIO DEBUG: Verificando condi��es para convers�o PTT')
	}
	
	// Track converted file path for cleanup
	let convertedFilePath: string | undefined
	
	// ?? IMPORTANTE: Convers�o deve acontecer ANTES da encripta��o e upload!
	if (requiresAudioTransformation) {
		try {
			...
			const basePathForConversion = tempMedia.originalFilePath!.replace(/\.[^/.]+$/, '')
			convertedFilePath = basePathForConversion + '_ptt.ogg'
			...
			const possiblePaths = [
				...
				convertedFilePath.replace(/\.ogg$/, '.m4a')            // .m4a fallback
			]
			
			let actualConvertedPath = convertedFilePath
			...
			// Update media to use converted file
			uploadData.media = { url: actualConvertedPath }
			...
			// ?? IMPORTANTE: For�ar propriedades espec�ficas para PTT no WhatsApp
			uploadData.ptt = true
			
			logger?.debug({ 
				original: tempMedia.originalFilePath, 
				converted: actualConvertedPath,
				oldMimetype,
				newMimetype: uploadData.mimetype,
				ptt: uploadData.ptt,
				codecUsed: actualConvertedPath.endsWith('.ogg') ? 'opus' : 'ipod',
				extensionUsed: actualConvertedPath.split('.').pop()
			}, '?? AUDIO CONVERSION: ? PR�-CONVERS�O conclu�da! Agora vai para upload')
	...
	// 🔧 FIX: Não deletar arquivos no .finally() - aguardar upload completo
	...
	// 🔧 FIX: Limpar arquivos temporários APENAS DEPOIS que o upload terminou
	try {
...
export const generateWAMessageContent = async (
	...
	if ('buttons' in message && !!message.buttons) {
		...
		if ('text' in message) {
			...
			buttonsMessage.headerType = type ? (ButtonType as any)[type] : ButtonType.EMPTY
