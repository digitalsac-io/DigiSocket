...
// @ts-ignore - ffmpeg-static n�o tem tipos TypeScript
import ffmpegPath from 'ffmpeg-static'
// @ts-ignore - @ffprobe-installer/ffprobe n�o tem tipos TypeScript
...
/** 
 * Converts audio to PTT-compatible format using MediaUtils logic (simplified and tested)
 * Based on production logic from DigitalSac backend - PROVEN TO WORK
 */
export const convertAudioToPTTFormat = async (
	inputPath: string,
	outputPath: string,
	logger?: ILogger
): Promise<void> => {

	return await convertAudioToBaileysStyle(inputPath, outputPath, true, logger) // true = isPTT
}
...
async function convertAudioToBaileysStyle(
	inputPath: string,
	outputPath: string,
	isPTT: boolean,
	logger?: ILogger
	...
	return new Promise((resolve, reject) => {
		const format = isPTT ? 'ogg' : 'mp3'
		const audioCodec = isPTT ? 'libopus' : 'libmp3lame'
		const audioBitrate = isPTT ? '48k' : '128k'
		const audioChannels = isPTT ? 1 : 2
		...
		const stream = command.pipe() as NodeJS.ReadableStream
...
export const getStream = async (item: WAMediaUpload, opts?: AxiosRequestConfig) => {
	...
	if (urlStr.startsWith('http://') || urlStr.startsWith('https://')) {
		return { stream: await getHttpStream(item.url, opts as any), type: 'remote' } as const
	}
...
export const encryptedStream = async (
	...
	const { stream, type } = await getStream(media, opts as any)
